    ...
    mouse(e, pos, node) {
        if (e.type === 'pointerdown' || (e.type === 'pointermove' && this.activeIdx !== -1)) {
            ...
            if (this.activeIdx !== -1) {
                ...
                this.value[this.activeIdx] = val;
                node.setDirtyCanvas(true, true);
                if (node.onWidgetChanged) node.onWidgetChanged(this.name, this.value, this.value, this);
                return true;
            }
        } else if (e.type === 'pointerup') {
            this.activeIdx = -1;
    ...
    getCustomWidgets() {
        return {
            VEC2POS: (node, inputName, inputData) => {
                const w = new Vec2PosWidget(inputName, inputData);
                if (inputData.default) w.value = [...inputData.default];
                return {
                    widget: node.addCustomWidget(w),
                    minWidth: 200,
                    minHeight: 180,
                }
            },
            VEC3POS: (node, inputName, inputData) => {
                const w = new Vec3PosWidget(inputName, inputData);
                if (inputData.default) w.value = [...inputData.default];
                return {
                    widget: node.addCustomWidget(w),
                    minWidth: 200,
            ...
            VEC3: (node, inputName, inputData) => {
                // Determine if this is a color or just a vec3
                if (inputName.toLowerCase().includes("color") || inputName.toLowerCase().includes("start") || inputName.toLowerCase().includes("end")) {
                    const w = new Vec3ColorWidget(inputName, inputData);
                    if (inputData.default) w.value = [...inputData.default];
                    return {
                        widget: node.addCustomWidget(w),
                        minWidth: 200,
            ...
            VEC4COLOR: (node, inputName, inputData) => {
                const w = new Vec4ColorWidget(inputName, inputData);
                if (inputData.default) w.value = [...inputData.default];
                return {
                    widget: node.addCustomWidget(w),
                    minWidth: 200,
